...
setTimeout(() => {
  ...
  try {
    ...
    assert(!!tb, '底部工具栏已挂载');
    assert(tb.style.bottom === '' || tb.style.bottom === '0px', '初始 bottom=0 (' + tb.style.bottom + ')');
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        assert(parseInt(tb.style.bottom) === 0 || tb.style.bottom === '', '键盘收起后 bottom 归零 (' + tb.style.bottom + ')');
