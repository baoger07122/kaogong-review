...
App.Cloud = (function () {
  ...
  let _queue = new Map(); // 待推送任务；同一记录只保留最后一次操作
  let _flushing = false;
  ...
  function queuePush(coll, item) {
    if (!item || !item.id) return;
    const job = { op: 'put', coll, item };
    _queue.set(jobKey(job), job);
    scheduleFlush();
  }

  function queueDelete(coll, id) {
    if (!id) return;
    const job = { op: 'del', coll, id };
    _queue.set(jobKey(job), job);
    scheduleFlush();
  }

  function scheduleFlush(delay = 500) {
    if (_flushing || _flushTimer) return;
  ...
  async function flush() {
    if (_flushing) return;
    _flushing = true;
    const batch = Array.from(_queue.values());
    _queue.clear();
    for (const job of batch) {
      try {
        ...
        _queue.set(jobKey(job), job); // 失败重试
      }
    }
    _flushing = false;
    if (_queue.size) scheduleFlush(2000);
  }
    ...
    for (const coll of COLLS) {
      ...
      try {
        for (const r of remote) {
          ...
          if (!l || t(r) > t(l)) {
            toPut.push(r);                       // 云端新增/更新 → 批量写入本地
          }
          // 本地更新/更早 → 保留本地（已在上一步推上云）
        }
        await App.DB.putMany(coll, toPut);
      } finally {
        _suppress = false;
      }
    }
  ...
  function wrapDB() {
    const orig = {
      add: App.DB.add,
      put: App.DB.put,
      putMany: App.DB.putMany,
      replaceStores: App.DB.replaceStores,
      remove: App.DB.remove
    };
    ...
    App.DB.putMany = async function (store, items) {
      const r = await orig.putMany(store, items);
      (items || []).forEach(item => _onMutate(store, 'put', item));
      return r;
    };
    App.DB.replaceStores = async function (replacements) {
      const r = await orig.replaceStores(replacements);
      Object.entries(replacements || {}).forEach(([store, items]) => {
        (items || []).forEach(item => _onMutate(store, 'put', item));
      });
      return r;
    };
