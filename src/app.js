...
App.Utils = {
  ...
  simpleMarkdown(text) {
    ...
    while (i < lines.length) {
      ...
      // 标题（兼容行首残留 &amp;#35; 被解码后的 #；纯 # 标记无文字的行直接跳过，避免显示裸 "###"）
      ...
      if (hMatch) {
        if (hMatch[2].trim()) {
          ...
          out.push('<' + tag + ' class="md-preview-' + tag + '">' + this._renderInlineMarkdown(htext) + '</' + tag + '>');
        }
        i++; continue;
      }
      ...
      if (/^[\-\*]\s+(.*)$/.test(trimmed)) {
        ...
        out.push('<ul class="md-preview-ul">' + items.map(it => '<li>' + this._renderInlineMarkdown(it) + '</li>').join('') + '</ul>');
        continue;
      }
      ...
      if (/^\d+\.\s+(.*)$/.test(trimmed)) {
        ...
        out.push('<ol class="md-preview-ol">' + items.map(it => '<li>' + this._renderInlineMarkdown(it) + '</li>').join('') + '</ol>');
        continue;
      }
      ...
      if (/^>\s?(.*)$/.test(trimmed)) {
        ...
        out.push('<blockquote class="md-preview-blockquote">' + items.map(it => this._renderInlineMarkdown(it)).join('<br>') + '</blockquote>');
        continue;
      }

      // 普通段落（合并连续非空行；块内软换行保留为 <br>，不再被空格吞掉）
      const paraLines = [];
      while (i < lines.length && lines[i].type === 'normal' && lines[i].content.trim() !== '') {
        paraLines.push(lines[i].content);
        i++;
      }
      const para = paraLines.join('\n').trim();
      if (para) {
        out.push('<p class="md-preview-p">' + this._renderInlineMarkdown(para).replace(/\n/g, '<br>') + '</p>');
      }
    }
...
App.DB = (function() {
  ...
  async function getBackupInterval() {
    const v = await kvGet('icloud_backup_interval_days');
    return v ? parseInt(v, 10) : 0;
  }
  async function setBackupInterval(days) {
    await kvSet('icloud_backup_interval_days', parseInt(days, 10) || 0);
  }
  async function isBackupDue() {
    const days = await getBackupInterval();
    if (days <= 0) return false;
    const last = await kvGet('last_icloud_backup');
    if (!last) return true;
    return Date.now() - parseInt(last, 10) > days * 24 * 60 * 60 * 1000;
  }

  // ===== 自动本地备份（静默，存 IndexedDB，最多保留 5 份）=====
  ...
  async function autoBackup() {
    ...
    const now = Date.now();
    if (_autoBacking) return null;
    if (now - _lastAutoBackup < 3 * 60 * 1000) return null; // 节流：至少 3 分钟一次
    _lastAutoBackup = now;
    _autoBacking = true;
    try {
      const data = await buildBackupData();
      const ts = Date.now();
      await kvSet(AUTO_BACKUP_PREFIX + ts, { ts, data });
      await kvSet('last_auto_backup', ts);
      const all = await getAll('keyvalue');
      const keys = all.map(r => r.key).filter(k => k && k.startsWith(AUTO_BACKUP_PREFIX));
      if (keys.length > AUTO_BACKUP_MAX) {
        const sorted = keys.map(k => parseInt(k.slice(AUTO_BACKUP_PREFIX.length), 10)).sort((a, b) => a - b);
        const removeCnt = keys.length - AUTO_BACKUP_MAX;
        for (let i = 0; i < removeCnt; i++) {
          await remove('keyvalue', AUTO_BACKUP_PREFIX + sorted[i]);
        }
      }
      return ts;
    } catch (e) {
  ...
  async function listAutoBackups() {
    const all = await getAll('keyvalue');
    return all
      .filter(r => r.key && r.key.startsWith(AUTO_BACKUP_PREFIX))
      .map(r => ({ key: r.key, ts: (r.value && r.value.ts) || 0 }))
      .sort((a, b) => b.ts - a.ts);
  }

  async function restoreAutoBackup(key) {
    const item = await get('keyvalue', key);
    if (!item || !item.value || !item.value.data) throw new Error('备份不存在');
    await importAll(item.value.data);
  }

  function setAutoBackupEnabled(v) {
    _autoBackupEnabled = !!v;
    kvSet('auto_backup_enabled', _autoBackupEnabled ? '1' : '0');
  }

  async function initAutoBackupFlag() {
    try {
      const v = await kvGet('auto_backup_enabled');
      _autoBackupEnabled = (v !== '0');
    } catch (e) { _autoBackupEnabled = true; }
  ...
  return {
    ...
    getStats,
    exportAll, importAll,
    buildBackupData, shareBackupToICloud, getBackupInterval, setBackupInterval, isBackupDue,
    autoBackup, listAutoBackups, restoreAutoBackup,
    setAutoBackupEnabled, initAutoBackupFlag,
    kvGet, kvSet,
    isAvailable
  };
})();
...
App.Components = {
  ...
  sketchPad(opts) {
    ...
    const penSizes = [ {n:'细', v:1.75}, {n:'中', v:3.25}, {n:'粗', v:4.25} ];
    ...
    placeholder.className = 'sketch-pad__placeholder';
    placeholder.textContent = '在此用 Pencil 手写笔记';
    ...
    function updatePlaceholder() {
      placeholder.style.display = (isEmpty()) ? 'flex' : 'none';
    }
    function isEmpty() {
    ...
    function moveCursor(pos, radius) {
      cursor.style.left = pos.x / scaleFactor() + 'px';
      cursor.style.top = pos.y / scaleFactor() + 'px';
      cursor.style.width = (radius * 2) + 'px';
      cursor.style.height = (radius * 2) + 'px';
    }

    // 画笔：采样去重 + 远距离插值 + 压力平滑 + 中点贝塞尔，线条顺滑无圆点
    let lastX = 0, lastY = 0;
    ...
    function drawSmoothStroke(newPos, e) {
      ...
      // 压力平滑：低通滤波 + 钳制，避免 Apple Pencil 压力抖动导致线宽突变
      let p = 0.5;
      if (e && e.pointerType === 'pen' && e.pressure > 0) {
        p = e.pressure * 0.45 + lastPressure * 0.55;
        p = Math.min(1, Math.max(0.35, p));
      }
      lastPressure = p;
      ctx.lineWidth = Math.max(1, currentPenSize * (0.55 + p * 0.75) * sf);
      ...
      if (lastPt) {
        ...
        if (dist < 1.5) return;
        // 距离过远：按 12px 步长线性插值细分，避免远点直接拉弧线「连成一团」
        if (dist > 24) {
      ...
      points.push(newPos);
      if (points.length < 3) return; // 前两个点只记录，到第三个点开始曲线
      drawSegment();
    }
    ...
    function endDraw(e) {
      ...
      // 收尾：把最后一段连到终点（距离过近跳过，避免小圆点；单点点击不画圆点）
      if (mode !== 'eraser' && points.length > 0) {
        ...
        if (Math.sqrt(dx * dx + dy * dy) > 2) {
          ...
          ctx.lineJoin = 'round';
          const sf = scaleFactor();
          ctx.lineWidth = Math.max(1, currentPenSize * sf);
          ctx.beginPath();
          ctx.moveTo(lastX, lastY);
    ...
    if (window.ResizeObserver) {
      const ro = new ResizeObserver(() => { if (initialized) applySize(); });
      ro.observe(canvasWrap);
    } else {
      window.addEventListener('resize', () => { if (initialized) applySize(); });
    }
    ...
    return {
      ...
      clear: () => { ctx.clearRect(0, 0, canvas.width, canvas.height); undoStack = [ snapshot() ]; redoStack = []; baseHasContent = false; updatePlaceholder(); },
      undo: () => { if (undoStack.length > 1) { redoStack.push(undoStack.pop()); restore(undoStack[undoStack.length - 1]); updatePlaceholder(); } },
      redo: () => { if (redoStack.length > 0) { undoStack.push(redoStack.pop()); restore(undoStack[undoStack.length - 1]); updatePlaceholder(); } },
      setMode: (m) => setMode(m),
  ...
  notionEditor(initialContent, placeholder, onChange) {
    ...
    async function insertInternalLink(blockEl, kind) {
      try {
        ...
        if (editable) {
          ...
          const be = editable.closest('.notion-block');
          if (be) syncBlockData(be, blocks[parseInt(be.dataset.index)]);
          notifyChange();
        }
    ...
    function parseMarkdownToBlocks(md) {
      ...
      lines.forEach(line => {
        ...
        // 空行保留为空白块：编辑/查看往返时空白行不丢失（导出仍为空行，查看渲染为空白占位）
        if (line.trim() === '') { result.push(createBlock('text', '')); return; }
        // 纯 # 标记行（无文字）跳过，避免回编辑页残留裸 ###
        ...
        result.push({ type, content, html: renderInlineMarkdown(content), indent, color:'', checked: /\[x\]/.test(line), collapsed:false, emoji:'💡', summary:'' });
      });
      flushTable();
    ...
    function exportToMarkdown() {
      ...
      blocks.forEach(b => {
        ...
        switch(b.type) {
          ...
          case 'numbered': numCounter++; lines.push(pad + numCounter + '. ' + md); break;
...
App.Pages.Home = {
  async render(params) {
    ...
    progressBar.style.cssText = 'height:6px;background:var(--bg-tertiary);border-radius:9999px;overflow:hidden;margin-bottom:var(--spacing-md);';
    const progressFill = document.createElement('div');
    progressFill.id = 'todo-progress-fill';
    progressFill.style.cssText = 'height:100%;width:' + pct + '%;background:linear-gradient(90deg,#4A90E2,#34C759);border-radius:9999px;transition:width 0.3s ease;';
    ...
    addBtn.addEventListener('click', async () => {
      ...
      await App.DB.addTodo({ text, type: this.todoState.type, completed: false, createdAt: new Date().toISOString() });
      addInput.value = '';
...
App.Pages.Errors = {
  ...
  parseQuestion(raw) {
    ...
    if (circleIdx > 0 && circleLines.slice(circleIdx).every(l => /^[①②③④⑤⑥⑦⑧]/.test(l))) {
      return {
        question: circleLines.slice(0, circleIdx).join('\n'),
    ...
    if (lines.length >= 2) {
      ...
      return { question: lines.slice(0, qEnd + 1).join('\n'), options: lines.slice(qEnd + 1) };
    }
  ...
  async renderDetail(params) {
    ...
    if (detailRight) {
      ...
      doodleBtn.title = error.screenDoodle ? '屏幕勾画（已隐藏保存）' : '屏幕勾画';
      doodleBtn.innerHTML = '✏️' + (error.screenDoodle ? '<span class="detail-header-action__dot"></span>' : '');
    ...
    enoteContent.addEventListener('click', () => {
      ...
      const editor = App.Components.notionEditor(error.note, false, function (content) {
        error.note = content;
      });
    ...
    noteContent.addEventListener('click', () => {
      ...
      const editor = App.Components.notionEditor(error.analysisNote, '输入解析思路...（支持 Markdown 即时预览）', function (content) {
  ...
  renderForm(params) {
    ...
    const buildForm = () => {
      ...
      const editor = App.Components.notionEditor(formData.analysisNote, '输入解析思路...（支持 Markdown 即时预览）', function (content) {
      ...
      const enoteEditor = App.Components.notionEditor(formData.note, false, function (content) {
...
App.Pages.StudyStats = {
  ...
  async render(params) {
    ...
    todos.forEach(t => {
      ...
      if (!byDay[k]) byDay[k] = { total: 0, done: 0 };
      byDay[k].total++;
    ...
    header.innerHTML = `
      <div style="font-size:var(--font-3xl);font-weight:700;color:var(--text-primary);margin-bottom:var(--spacing-sm);">学习统计</div>
      <div style="font-size:var(--font-sm);color:var(--text-tertiary);line-height:1.6;">按日历查看每日待办的完成情况，绿色 = 全部完成，黄色 = 有未完成。</div>
    `;
    container.appendChild(header);
    ...
    prevBtn.className = 'btn btn--outline btn--sm'; prevBtn.textContent = '◀ 上月';
    const monthLabel = document.createElement('div');
    monthLabel.style.cssText = 'font-size:var(--font-lg);font-weight:600;';
    const nextBtn = document.createElement('button');
    nextBtn.className = 'btn btn--outline btn--sm'; nextBtn.textContent = '下月 ▶';
    const refresh = () => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      if (info && info.total > 0) {
        ...
        inner += '<div class="study-cal__stat">' + info.done + '/' + info.total + '</div>';
      } else {
    ...
    const summary = document.createElement('div');
    summary.style.cssText = 'padding:var(--spacing-md) var(--page-padding);';
    summary.innerHTML = `
      <div style="background:var(--bg-tertiary);border-radius:var(--radius-md);padding:var(--spacing-md);">
        <div style="font-weight:600;margin-bottom:6px;">📅 今日待办</div>
        <div style="display:flex;gap:var(--spacing-lg);font-size:var(--font-md);">
          <span>完成 <b style="color:var(--color-success);">${todayInfo ? todayInfo.done : 0}</b></span>
          <span>未完成 <b style="color:var(--color-warning);">${todayInfo ? todayInfo.total - todayInfo.done : 0}</b></span>
          <span>总计 <b>${todayInfo ? todayInfo.total : 0}</b></span>
        </div>
      </div>
    `;
...
App.Pages.Settings = {
  ...
  render(params) {
    ...
    backupGroup.innerHTML = `
      ...
      <div class="backup-hero__auto">
        <div class="settings-item" style="cursor:default">
          <span class="settings-item__label">自动备份提醒</span>
          <span class="settings-item__value">
            <select id="auto-backup-interval" style="font-size:var(--font-sm);padding:4px 8px;border-radius:var(--radius-sm);border:1px solid var(--divider-color);background:var(--bg-card);color:var(--text-primary);">
        ...
        <div class="backup-hero__hint" id="auto-backup-info">读取中…</div>
      </div>
    `;
    ...
    const shareBackupToICloud = async () => {
      try {
        ...
        const today = new Date().toISOString().slice(0, 10);
        const lastDay = last ? new Date(parseInt(last, 10)).toISOString().slice(0, 10) : '';
        if (lastDay === today) {
          const go = await App.Components.confirm(
            '今日已备份',
            '今天已经备份过一次。继续保存会用新数据覆盖 iCloud「考公备考系统-backup.json」中的旧数据，是否继续？',
            '覆盖保存', '取消', false
          );
          if (!go) return;
        }
    ...
    const refreshBackupInfo = async () => {
      try {
        const [interval, last] = await Promise.all([
          App.DB.getBackupInterval(),
          App.DB.kvGet('last_icloud_backup')
        ]);
        const sel = backupGroup.querySelector('#auto-backup-interval');
        if (sel) sel.value = String(interval || 0);
        const info = backupGroup.querySelector('#auto-backup-info');
        if (info) {
          const map = { 0: '已关闭', 1: '每天', 7: '每周', 30: '每月' };
          let txt = '自动备份：' + (map[interval] || '已关闭');
          if (last) {
    ...
    const intervalEl = backupGroup.querySelector('#auto-backup-interval');
    if (intervalEl) intervalEl.addEventListener('change', async (e) => {
      await App.DB.setBackupInterval(e.target.value);
      await refreshBackupInfo();
    });
    refreshBackupInfo();


    // ===== 云同步（折叠式：点击才展开登录，避免无后端时暴露未就绪状态）=====
    ...
    const cloudCardHTML = loggedIn ? `
      ...
      ` : `
        <div class="cloud-card">
          ...
          <div class="cloud-hint">需配套后端（Node 服务）才能使用。登录后保存即自动上传云端，可跨设备访问；换设备时点「立即同步」即可把云端数据合并到本机。</div>
        </div>
      `;
    cloudGroup.innerHTML = `
      <div class="fold-box">
        <button class="fold-head" type="button" id="cloud-fold-head">
          <span class="fold-title">☁️ 云同步</span>
          <span class="fold-status">${loggedIn ? '已登录' : '点击展开登录'}</span>
          <span class="fold-arrow">▾</span>
        </button>
        <div class="fold-body" id="cloud-fold-body" style="display:none;">
          ${cloudCardHTML}
        </div>
      </div>
    `;
    ...
    // 云同步折叠开合
    const cloudFoldHead = cloudGroup.querySelector('#cloud-fold-head');
    const cloudFoldBody = cloudGroup.querySelector('#cloud-fold-body');
    const cloudFoldArrow = cloudFoldHead.querySelector('.fold-arrow');
    cloudFoldHead.addEventListener('click', () => {
      const isOpen = cloudFoldBody.style.display !== 'none';
      cloudFoldBody.style.display = isOpen ? 'none' : 'block';
      cloudFoldArrow.classList.toggle('fold-open', !isOpen);
    });
    ...
    importItem.innerHTML = `
      ...
      <span class="settings-item__value">从单个 JSON 文件恢复 ›</span>
    `;
    ...
    tagSection.innerHTML = `
      ...
      <div style="padding:0 0 10px;font-size:var(--font-xs);color:var(--text-tertiary);line-height:1.5;">考点按「科目 → 模块」分级管理。错因不再设全局池，全部按模块独立管理（在「考点管理」中展开模块即可增删该模块的专属错因，模块间互不互通）。</div>
    `;
    ...
    // 内联修改标签
    const startEdit = (chip, module, oldName) => {
      ...
      saveBtn.addEventListener('click', async () => {
        const v = input.value.trim();
        if (!v) return;
        await App.Tags.renameKnowledgePoint(module, oldName, v);
        App.Pages.Settings.render({});
      });
    ...
    const buildKpChip = (module, name) => {
      ...
      editBtn.addEventListener('click', (e) => { e.stopPropagation(); startEdit(chip, module, name); });
      const delBtn = document.createElement('button');
    ...
    // 考点管理已移至独立页面（首页「考点管理」卡片入口）；此处仅保留跳转
    const kpEntry = document.createElement('div');
    kpEntry.className = 'settings-item';
    kpEntry.style.cssText = 'cursor:pointer;';
    kpEntry.innerHTML = '<span class="settings-item__label">🗂️ 考点管理（科目 / 模块 / 错因）</span><span class="settings-item__value" style="color:var(--color-primary)">进入 ›</span>';
    kpEntry.addEventListener('click', () => App.Router.navigate('kpmanage'));
    content.appendChild(kpEntry);
    ...
    aboutGroup.innerHTML = `
      ...
      <div class="settings-item">
        ...
        <span class="settings-item__value">${App.VERSION}</span>
      </div>
      <div class="settings-item">
...
App.Pages.KpManage = {
  render(params) {
    ...
    container.innerHTML = '';
    this._state = this._state || { expanded: {} };
    ...
    header.style.cssText = 'padding:var(--spacing-lg) var(--page-padding);padding-bottom:var(--spacing-sm);';
    header.innerHTML = `
      <div style="font-size:var(--font-3xl);font-weight:700;color:var(--text-primary);margin-bottom:var(--spacing-sm);">考点管理</div>
      <div style="font-size:var(--font-sm);color:var(--text-tertiary);line-height:1.6;">
        考点按「科目 → 模块」分级管理；错因<b>按模块独立管理</b>，在模块内增删的错因仅属于该模块、与其它模块互不互通。
      </div>
    `;
    container.appendChild(header);

    const tree = document.createElement('div');
    tree.className = 'kp-tree is-managed';
    tree.style.padding = '0 var(--page-padding) var(--spacing-xl)';
    container.appendChild(tree);

    // 模块错因：chip（仅管理态下显示修改 / 删除）
    const buildModuleCauseChip = (module, reason) => {
      const chip = document.createElement('div');
      chip.className = 'kp-chip kp-chip--sub';
      chip.dataset.name = reason;
      chip.dataset.kind = 'ec';
      const nameEl = document.createElement('span');
      nameEl.className = 'kp-chip__name';
      nameEl.textContent = reason;
      chip.appendChild(nameEl);
      if (expanded['manage:' + module]) {
        const actions = document.createElement('span');
        actions.className = 'kp-chip__actions';
        const editBtn = document.createElement('button');
        editBtn.className = 'kp-chip__btn kp-chip__edit';
        editBtn.textContent = '修改';
        editBtn.addEventListener('click', (e) => { e.stopPropagation(); startModuleCauseEdit(chip, module, reason); });
        const delBtn = document.createElement('button');
        delBtn.className = 'kp-chip__btn kp-chip__del';
        delBtn.textContent = '删除';
        delBtn.addEventListener('click', async (e) => {
          e.stopPropagation();
          const ok = await App.Components.confirm('删除错因', '确定删除该模块错因「' + reason + '」？', '删除', '取消', true);
          if (ok) { await App.Tags.removeModuleErrorCause(module, reason); App.Pages.KpManage.render({}); }
        });
        actions.appendChild(editBtn);
        actions.appendChild(delBtn);
        chip.appendChild(actions);
    ...
    const startModuleCauseEdit = (chip, module, oldName) => {
      chip.classList.add('kp-chip--editing');
      chip.innerHTML = '';
      const input = document.createElement('input');
      input.className = 'kp-edit-input';
      ...
      saveBtn.addEventListener('click', async () => {
        const v = input.value.trim();
        if (v) await App.Tags.renameModuleErrorCause(module, oldName, v);
        App.Pages.KpManage.render({});
      });
      cancelBtn.addEventListener('click', () => App.Pages.KpManage.render({}));
      input.addEventListener('keydown', (e) => { if (e.key === 'Enter') saveBtn.click(); else if (e.key === 'Escape') cancelBtn.click(); });
      chip.appendChild(input); chip.appendChild(saveBtn); chip.appendChild(cancelBtn);
      input.focus(); input.select();
    };

    // 考点：chip（仅管理态下显示修改 / 删除，平时保持简洁）
    const buildKpChip = (module, name) => {
      const chip = document.createElement('div');
      chip.className = 'kp-chip';
      chip.dataset.name = name;
      chip.dataset.kind = 'kp';
      const nameEl = document.createElement('span');
      nameEl.className = 'kp-chip__name';
      nameEl.textContent = name;
      chip.appendChild(nameEl);
      if (expanded['manage:' + module]) {
        ...
        editBtn.className = 'kp-chip__btn kp-chip__edit';
        editBtn.textContent = '修改';
        editBtn.addEventListener('click', (e) => { e.stopPropagation(); startKpEdit(chip, module, name); });
        const delBtn = document.createElement('button');
        delBtn.className = 'kp-chip__btn kp-chip__del';
        delBtn.textContent = '删除';
        delBtn.addEventListener('click', async (e) => {
          e.stopPropagation();
          const ok = await App.Components.confirm('删除考点', '确定删除考点「' + name + '」？已录入错题 / 笔记中的该标签也会一并移除。', '删除', '取消', true);
          if (ok) { await App.Tags.removeKnowledgePoint(module, name); App.Pages.KpManage.render({}); }
        });
        actions.appendChild(editBtn);
        actions.appendChild(delBtn);
        chip.appendChild(actions);
    ...
    // ===== 长按拖拽排序（考点/错因 chip，管理态下生效）=====
    function bindChipDrag(chip, module, kind, wrap) {
      let timer = null, dragging = false, pointerId = null, lastY = 0;
      let ghost = null, ph = null;
      chip.addEventListener('pointerdown', (e) => {
        ...
        timer = setTimeout(() => {
          ...
          ghost.classList.remove('kp-chip--ghost');
          ghost.classList.add('kp-chip--ghost');
          ghost.style.width = rect.width + 'px';
          ghost.style.height = rect.height + 'px';
          ghost.style.top = (rect.top - wrapRect.top + wrap.scrollTop) + 'px';
          ghost.style.left = (rect.left - wrapRect.left) + 'px';
          ph = document.createElement('div');
          ph.className = 'kp-chip kp-chip--placeholder';
          ph.style.width = rect.width + 'px';
          ph.style.height = rect.height + 'px';
          wrap.appendChild(ghost);
          chip.style.visibility = 'hidden';
          wrap.insertBefore(ph, chip);
        }, 500);
      });
      const onMove = (ev) => {
        if (ev.pointerId !== pointerId) return;
        ...
        lastY = ev.clientY;
        const siblings = Array.from(wrap.children).filter(c => c !== ghost && c !== ph && !c.classList.contains('kp-empty'));
        let inserted = false;
        for (let i = 0; i < siblings.length; i++) {
          const rc = siblings[i].getBoundingClientRect();
          if (ev.clientY < rc.top + rc.height / 2) { wrap.insertBefore(ph, siblings[i]); inserted = true; break; }
        }
        if (!inserted) wrap.appendChild(ph);
      };
      const cleanup = (ev, finish) => {
        if (ev.pointerId !== pointerId) return;
        clearTimeout(timer);
        document.removeEventListener('pointermove', onMove);
        document.removeEventListener('pointerup', onEnd);
        ...
        if (ph) { ph.replaceWith(chip); ph = null; chip.style.visibility = ''; }
        if (finish) {
          const order = Array.from(wrap.children)
            .filter(c => c.classList.contains('kp-chip') && !c.classList.contains('kp-chip--placeholder') && c.dataset.name)
            .map(c => c.dataset.name);
          if (order.length > 1) {
            App.Tags.setOrder(module, kind, order).then(() => App.Components.toast('已排序', 'success'));
          }
        }
      };
      const onEnd = (ev) => cleanup(ev, true);
      const onCancel = (ev) => cleanup(ev, false);
      document.addEventListener('pointermove', onMove);
      document.addEventListener('pointerup', onEnd);
      document.addEventListener('pointercancel', onCancel);
    }

    const startKpEdit = (chip, module, oldName) => {
      chip.classList.add('kp-chip--editing');
      chip.innerHTML = ' ';
      const input = document.createElement('input');
      input.className = 'kp-edit-input';
      ...
      saveBtn.addEventListener('click', async () => {
        const v = input.value.trim();
        if (v) await App.Tags.renameKnowledgePoint(module, oldName, v);
        App.Pages.KpManage.render({});
      });
      cancelBtn.addEventListener('click', () => App.Pages.KpManage.render({}));
      input.addEventListener('keydown', (e) => { if (e.key === 'Enter') saveBtn.click(); else if (e.key === 'Escape') cancelBtn.click(); });
    ...
    const buildModuleBlock = (mod) => {
      ...
      modHead.className = 'kp-module__head';
      const kps = App.Tags.getKnowledgePoints(mod);
      const causes = App.Tags.getModuleErrorCauses(mod);
      modHead.innerHTML = '<span class="kp-arrow">▸</span><span class="kp-module__title">' + mod + '</span><span class="kp-count">' + kps.length + ' 考点 · ' + causes.length + ' 错因</span>';
      const manageBtn = document.createElement('button');
      manageBtn.className = 'kp-manage-btn' + (expanded['manage:' + mod] ? ' is-active' : '');
      manageBtn.textContent = expanded['manage:' + mod] ? '完成' : '✎ 管理';
      manageBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        expanded['m:' + mod] = true;
        expanded['manage:' + mod] = !expanded['manage:' + mod];
        App.Pages.KpManage.render({});
      });
      modHead.appendChild(manageBtn);
      modHead.addEventListener('click', () => { expanded['m:' + mod] = !expanded['m:' + mod]; App.Pages.KpManage.render({}); });
      modEl.appendChild(modHead);
      const modBody = document.createElement('div');
      modBody.className = 'kp-module__body';
      ...
      kpSection.className = 'kp-subsection';
      const kpLabel = document.createElement('div');
      kpLabel.className = 'kp-subsection__label';
      kpLabel.textContent = '📍 考点';
      ...
      if (kps.length === 0) {
        ...
        kps.forEach(kp => {
          const c = buildKpChip(mod, kp);
          chipsWrap.appendChild(c);
          if (expanded['manage:' + mod]) bindChipDrag(c, mod, 'kp', chipsWrap);
        });
      }
      kpSection.appendChild(chipsWrap);
      const addRow = document.createElement('div');
      addRow.className = 'kp-add-row';
      const addInput = document.createElement('input');
      addInput.className = 'form-input';
      ...
      addRow.appendChild(addInput);
      addRow.appendChild(addBtn);
      kpSection.appendChild(addRow);
      modBody.appendChild(kpSection);

      // —— 该模块错因区 ——
      const causeSection = document.createElement('div');
      causeSection.className = 'kp-subsection';
      const causeLabel = document.createElement('div');
      causeLabel.className = 'kp-subsection__label';
      causeLabel.textContent = '⚠️ 该模块错因';
      ...
      if (causes.length === 0) {
        const empty = document.createElement('span');
        empty.className = 'kp-empty';
        empty.textContent = '暂无模块错因';
        causeChipsWrap.appendChild(empty);
      } else {
        causes.forEach(ec => {
          const c = buildModuleCauseChip(mod, ec);
          causeChipsWrap.appendChild(c);
          if (expanded['manage:' + mod]) bindChipDrag(c, mod, 'ec', causeChipsWrap);
        });
      }
      causeSection.appendChild(causeChipsWrap);
      ...
      causeInput.placeholder = '新增该模块错因...';
      ...
      causeAddBtn.addEventListener('click', async () => {
        ...
        if (!v) return;
        await App.Tags.addModuleErrorCause(mod, v);
        App.Pages.KpManage.render({});
      });
      ...
      causeSection.appendChild(causeAddRow);
      modBody.appendChild(causeSection);
    ...
    App.Constants.SUBJECTS.forEach(subject => {
      ...
      const subjHead = document.createElement('div');
      subjHead.className = 'kp-subject__head';
      subjHead.innerHTML = '<span class="kp-arrow">▸</span><span class="kp-subject__icon">' + subject.icon + '</span><span class="kp-subject__title">' + subject.name + '</span>';
      subjHead.addEventListener('click', () => { expanded['s:' + subject.name] = !expanded['s:' + subject.name]; App.Pages.KpManage.render({}); });
      subjEl.appendChild(subjHead);
      const subjBody = document.createElement('div');
      subjBody.className = 'kp-subject__body';
      App.Constants.getModules(subject.name).forEach(mod => subjBody.appendChild(buildModuleBlock(mod)));
      subjEl.appendChild(subjBody);
      tree.appendChild(subjEl);
    });
  }
