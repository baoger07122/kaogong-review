...
App.Components = {
  ...
  tagManager(container, opts) {
    ...
    const state = { edit: false, expanded: {} };   // expanded: { 'kp:模块名': true }
    ...
    function collectModules() {
      const mods = [];
      App.Constants.SUBJECTS.forEach(s => {
        App.Constants.getModules(s.name).forEach(m => {
    ...
    const expKey = (kind, mod) => kind + ':' + mod;
    const isExpanded = (kind, mod) => state.expanded[expKey(kind, mod)] !== false;   // 默认展开
    ...
    function render() {
      ...
      title.textContent = opts.title || '管理标签';
      ...
      modules.forEach(mod => {
        ...
        headLeft.className = 'tag-module-header__left';
        const arrow = document.createElement('span');
        arrow.className = 'tag-module-arrow' + (isExpanded('kp', mod) ? ' expanded' : '');
        arrow.textContent = '▶';
        const nameEl = document.createElement('span');
        nameEl.className = 'tag-module-name';
        nameEl.textContent = mod;
        headLeft.appendChild(arrow);
        headLeft.appendChild(nameEl);
        // 数量徽章（考点+错因总数）
        let total = 0;
        kinds.forEach(k => { total += KIND_META[k].get(mod).length; });
        const countEl = document.createElement('span');
        countEl.className = 'tag-module-count';
        countEl.textContent = total + ' 个';
        modHead.appendChild(headLeft);
        modHead.appendChild(countEl);
        // 折叠切换：点击头部任意处
        modHead.addEventListener('click', () => {
          const anyExpanded = kinds.some(k => isExpanded(k, mod));
          kinds.forEach(k => { state.expanded[expKey(k, mod)] = !anyExpanded; });
          render();
        });
        modEl.appendChild(modHead);

        // 内容：每类标签一个区块（标签云 + 添加输入行）
        const modContent = document.createElement('div');
        modContent.className = 'tag-module-content';
        const anyExpanded = kinds.some(k => isExpanded(k, mod));
        if (anyExpanded) modContent.classList.add('expanded');
        ...
        kinds.forEach(kind => {
          ...
          secLabel.className = 'tag-subsection-label';
          secLabel.textContent = meta.label;
          modContent.appendChild(secLabel);
          ...
          if (tagNames.length === 0) {
            ...
            tagNames.forEach((name) => {
              ...
              pill.draggable = state.edit;
              const nameSpan = document.createElement('span');
              nameSpan.className = 'tag-pill__name';
              ...
              if (state.edit) {
                ...
                delBtn.addEventListener('click', async (e) => {
                  ...
                  if (ok) {
                    await meta.remove(mod, name);
                    render();
                  }
                });
                pill.appendChild(handle);
                pill.appendChild(delBtn);
                bindDrag(pill, kind, mod, cloud);
              }
      ...
      if (state.edit) {
        ...
        hint.textContent = '💡 按住标签右侧 ⋮⋮ 拖动排序，点击 × 删除标签，+ 添加 新增标签（操作即时保存）';
        container.appendChild(hint);
      }
    }

    // 同模块内拖拽排序（HTML5 DnD；顺序即时持久化）
    // 拖拽数据用组件级共享变量传递（源 pill dragstart 写入、目标 pill drop 读取；
    // 不依赖 dataTransfer，jsdom/低版本浏览器更稳，真实浏览器两者一致）
    let sharedDragData = null;
    function bindDrag(pill, kind, mod, cloud) {
      ...
      pill.addEventListener('drop', async (e) => {
        ...
        const data = sharedDragData;   // 源 pill dragstart 时写入
        sharedDragData = null;
        if (!data) return;
