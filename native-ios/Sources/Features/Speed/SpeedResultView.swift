...
struct SpeedResultView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            ScrollView {
                VStack(spacing: 0) {
                    Text(standard).font(.system(size: 12)).foregroundStyle(.secondary)
                        .padding(.top, 8)
                    VStack(spacing: 4) {
        ...
        .safeAreaInset(edge: .bottom, spacing: 0) {
            HStack(spacing: 10) {
                footerButton("重来", color: Color(red: 243 / 255.0, green: 244 / 255.0, blue: 246 / 255.0), ink: ink, action: onRestart)
                footerButton("复练", color: Color(red: 239 / 255.0, green: 246 / 255.0, blue: 255 / 255.0), ink: blue, action: onRetry)
    ...
    private func tableHeader(width: CGFloat) -> some View {
        ...
        .foregroundStyle(ink).padding(.vertical, 10)
    ...
    private func tableRow(_ question: SpeedQuestion, number: Int, width: CGFloat) -> some View {
        ...
        return HStack(spacing: 0) {
            cell("\(number)", width: 40, size: 13, color: .secondary)
            ...
            cell(String(format: "%.1fs", question.timeUsed), width: 56, size: 13, color: .secondary)
        }
        .padding(.vertical, 12)
    ...
    private func footerButton(_ title: String, color: Color, ink: Color, bold: Bool = false, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Text(title).font(.system(size: 15, weight: bold ? .bold : .medium))
                .foregroundStyle(ink).frame(maxWidth: .infinity).padding(.vertical, 13)
