...
struct SpeedKeypadMetrics {
    ...
    static func practiceSizing(contentHeight: CGFloat, viewportHeight: CGFloat) -> (keyboard: CGFloat, footer: CGFloat) {
        ...
        let footer = min(84, max(0, available - 280 - 140))
        let keyboard = min(min(340, max(280, viewportHeight * 0.42)), max(0, available - footer - 88))
