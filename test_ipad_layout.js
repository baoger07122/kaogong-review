...
let _vvHeight = 768, _vvListeners = {};
const dom = new JSDOM(html, {
  ...
  beforeParse(w) {
    ...
    Object.defineProperty(w, 'innerWidth', { value: 1024, writable: true, configurable: true });
    Object.defineProperty(w, 'innerHeight', { value: 768, writable: true, configurable: true });
...
setTimeout(() => {
  ...
  try {
    ...
    assert(cs.bottom === '16px', 'bottom 悬浮间距 16px (' + cs.bottom + ')');
    ...
    // 3. iPad 分屏检测
    console.log('\n[3] iPad 分屏检测');
    assert(doc.body.classList.contains('ipad-split') === false, 'iPad 全屏 1024px 不加 ipad-split');
    // 模拟分屏：宽度变 800
    Object.defineProperty(win, 'innerWidth', { value: 800, writable: true, configurable: true });
    win.dispatchEvent(new win.Event('resize'));
    setTimeout(() => {
      assert(doc.body.classList.contains('ipad-split'), '分屏 800px 加 ipad-split 类');
      // 还原
      Object.defineProperty(win, 'innerWidth', { value: 1024, writable: true, configurable: true });
      win.dispatchEvent(new win.Event('resize'));
      setTimeout(() => {
        assert(!doc.body.classList.contains('ipad-split'), '恢复 1024px 移除 ipad-split');
